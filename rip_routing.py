...
def new_router_arise(table,packet):
    src = packet['header'][2]
    if src not in table.keys():
        for m in range(len(packet['entry'])):
            if packet['entry'][m][1] == router_id:
                ...
                routing_algorithms(table, packet)
    else:
        routing_algorithms(table, packet)
    return table
...
def send_packet1(table):
    ...
    for port in output_ports.keys():
        ...
        outSock.sendto(json_packet, ('127.0.0.1', port))
...
def receive_packet(listen_list, table):
    ...
    new_table = table
    r, w, e = select.select(listen_list, [], [], 0)
    if r != []:
        sock = r[0]
        ...
        if isValid == True:
            new_table = new_router_arise(table,rec_packet)
        else:
...
def print_rtable(table):
    ...
    print('|' + 'Dst Router'.center(width, ' ') + '|' + 'Metric'.center(width, ' ')
           ...
           + '|' + 'GFlag'.center(width, ' ') + '|' + 'Garbage'.center(width, ' ') + '|')
    print('=' * 80)
    ...
    for key in dst_routers:
        ...
        print('|' + str(key).center(width, ' ') + '|' + str(contxt[0]).center(width, ' ')
               ...
               + '|'  + str(contxt[1]).center(width, ' ') + '|' + str(contxt[4]).center(width, ' ') + '|')
