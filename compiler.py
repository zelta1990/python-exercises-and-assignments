...
class Boolean_AST:
    def __init__(self,result, op):
        self.result = result
        self.op = op
    def __repr__(self):
        return repr(self.op)+repr(self.result)
    def indented(self, level):
        return indent(self.op, level) + \
               self.result.indented(level+1) 
    def true_code(self,label):
        op = {Token.OR, Token.AND,Token.NOT}
        l1 = label_generator.next()
        if self.op==Token.OR:
            st = ''                    
            for term in self.result:
                st+=term.true_code(label)
            return st
        elif self.op==Token.AND:   
            st = ''                    
            for factor in self.result:
                st+=factor.false_code(l1)
            st = st + 'goto '+label+'\n'+l1+':\n'            
            return st
        elif self.op == Token.NOT:
            st = ''                    
            return self.result.false_code(label)
    ...
    def false_code(self,label):
        ...
        if self.op==Token.OR:
            st = ''                            
            for term in self.result:
                st+=term.true_code(l1)
            st += 'goto ' + label + '\n' + l1 + ':\n'
            return st
        elif self.op==Token.AND: 
            st = ''                            
            for factor in self.result:
                st += factor.false_code(label)
            return st
        elif self.op == Token.NOT:
            st = ''                                        
            return self.result.true_code(label) 
...
def if_statement():
    ...
    if scanner.lookahead() == (Token.ELSE):
        ...
        return If_Else_AST(condition, then, else_statement)
    else:
        scanner.consume(Token.END)
...
def booleanExpression():
    ...
    while scanner.lookahead() in [Token.OR]:
        ...
        term = booleanTerm()
        result.append(term)
    return Boolean_AST(result, Token.OR)
...
def booleanTerm():
    ...
    return Boolean_AST(result, Token.AND)

def booleanFactor():
    if scanner.lookahead() == Token.NOT:
        scanner.consume(Token.NOT)
        result = booleanFactor()
        return Boolean_AST(result, Token.NOT)
    else:
        result = comparison()
        return result
